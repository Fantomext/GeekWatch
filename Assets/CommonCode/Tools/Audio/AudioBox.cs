...
namespace Audio
{
    public class AudioBox : MonoBehaviour
    {
        ...
        void AddMusic(Sound s) => AddSouce(s, musicMixer);
        void AddSound(Sound s) => AddSouce(s, soundMixer);

        //Adding AudioSource with Sound prefs to AudioSystem
        void AddSouce(Sound s, AudioMixerGroup mixerGroup)
        {
            s.source = gameObject.AddComponent<AudioSource>();
            s.source.clip = s.clip;

            s.SetMixerGroup(mixerGroup);
